...
for i in range(images.shape[0]):
    io.imshow(tile(images[0], [3]))
    result = network.predict(images[i])
    preClass = result[0].tolist().index(max(result[0]))
    gtClass = labels[i].tolist().index(max(labels[i]))
    if preClass==gtClass:
        num = num+1
    print('%s   %s   %s'%(preClass,gtClass,preClass==gtClass))
